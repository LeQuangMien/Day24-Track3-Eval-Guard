...
def pii_scan(text: str, analyzer=None, anonymizer=None) -> dict:
    ...
    # Lọc bỏ low-confidence false positives: model NER (PHONE_NUMBER, US_*)
    # chạy bằng pipeline "en" trên text tiếng Việt cho score thấp, không tin cậy.
    MIN_SCORE = 0.5
    # PERSON dùng spacy NER tiếng Anh áp lên text tiếng Việt — không đáng tin
    # cậy dù score cao (vd "nghỉ phép" bị nhận nhầm thành PERSON). Loại trừ
    # hẳn entity type này, dựa vào EMAIL_ADDRESS + custom VN_CCCD/VN_PHONE.
    UNRELIABLE_TYPES = {"PERSON"}
    results = [r for r in results
               if r.score >= MIN_SCORE and r.entity_type not in UNRELIABLE_TYPES]
...
async def check_input_rail(text: str, rails=None) -> dict:
    ...
    # Chỉ chạy input rails (tắt dialog/output/retrieval): không cần dialog flow
    # định nghĩa cách trả lời mới biết input có bị chặn hay không, và tránh phải
    # đoán qua nội dung response (vốn rỗng khi không có dialog flow nào match).
    ...
    try:
        ...
        if os.getenv("GUARD_DEBUG"):
            import traceback
            print(f"  [DEBUG] EXCEPTION in generate_async for input={text[:50]!r}:")
            traceback.print_exc()
        # Fail-safe: nếu NeMo lỗi, không thể xác định an toàn → coi như chặn
        return {"allowed": False, "blocked_reason": "nemo_error", "response": str(e)}

    # Với GenerationOptions, response là GenerationResponse object (.response, .log)
    if isinstance(result.response, list):
        response_text = "\n".join(
            m.get("content", "") for m in result.response if isinstance(m, dict)
        )
    else:
        response_text = str(result.response)

    # Cách đáng tin cậy nhất: đọc trực tiếp activated_rails trong log thay vì
    # đoán qua nội dung response — input rail bị "stop" nghĩa là đã chặn.
    blocked = False
    activated_names = []
    if result.log and result.log.activated_rails:
        for rail in result.log.activated_rails:
            activated_names.append(getattr(rail, "name", None))
            if getattr(rail, "type", None) == "input" and getattr(rail, "stop", False):
                blocked = True

    if os.getenv("GUARD_DEBUG"):
        print(f"  [DEBUG] input={text[:50]!r}")
        print(f"  [DEBUG]   response_text={response_text[:200]!r}")
        print(f"  [DEBUG]   activated_rails={activated_names!r} blocked={blocked}")
...
async def check_output_rail(question: str, answer: str, rails=None) -> dict:
    ...
    # Chỉ chạy output rails (tắt dialog) — message cuối là assistant message
    # cần kiểm tra (output rail trong Colang 1.0 hoạt động trên bot_message).
    options = GenerationOptions(
    ...
    if result.log and result.log.activated_rails:
        for rail in result.log.activated_rails:
            if getattr(rail, "type", None) == "output" and getattr(rail, "stop", False):
                flagged = True

    if os.getenv("GUARD_DEBUG"):
        print(f"  [DEBUG output] answer={answer[:50]!r} flagged={flagged} response_text={response_text[:200]!r}")
...
def save_guard_report(pii_demo: dict, adversarial_results: list[dict],
                       latency: dict, path: str = "reports/guard_results.json") -> None:
    """Save Phase C report to JSON."""
    os.makedirs(os.path.dirname(path), exist_ok=True)
    total = len(adversarial_results)
    passed = sum(1 for r in adversarial_results if r["passed"]) if total else 0
    report = {
...
if __name__ == "__main__":
    # Setup engines một lần — tái sử dụng cho toàn bộ suite (NeMo init chậm)
    print("Setting up Presidio + NeMo Guardrails...")
    analyzer, anonymizer = setup_presidio()
    rails = setup_nemo_rails()
    ...
    save_guard_report(pii_demo, results, latency)